...
How to work in this repository. The product brief, the data model, and the build order live in [CLAUDE.md](CLAUDE.md),
...
| Dictionary       | CC-CEDICT, jieba frequencies, Unihan readings and Vietnamese Wiktionary meanings, vendored into `public/dict` |
...
Single Nuxt app, flat. Not a monorepo. Plain pnpm. One-off scripts live in `scripts/`.
...
pnpm lint                 # eslint, about what the code does
pnpm format               # prettier --write ., about how it looks
pnpm format:check         # prettier --check ., what CI should run
...
`pnpm dict:build` downloads a few hundred megabytes from mdbg.net, raw.githubusercontent.com, unicode.org and
kaikki.org, and takes several minutes; the Vietnamese Wiktionary extract is most of both numbers. The output is
committed and is about 9 MB, so this only needs running when the sources change. Pass `--cedict`, `--jieba`, `--unihan`
and `--envi` to build from local copies.

**A fresh clone has an empty local database.** `pnpm db:migrate:local` is not optional: without it every route returns
...
the whole argument: 120 columns, single quotes, semicolons, no trailing commas, LF, and `proseWrap: always` so markdown
paragraphs are rewrapped rather than hand wrapped by whoever typed them last. `prettier-plugin-tailwindcss` sorts class
lists, pointed at `app/assets/css/main.css` because Tailwind v4 keeps its config in the stylesheet.

- Run `pnpm format` before committing. Do not hand align anything; it will be undone.
- **ESLint no longer formats.** `eslint-config-prettier` is appended last in `eslint.config.mjs` and switches off every
  stylistic rule, so the two cannot disagree. Rules that remain are about behaviour: unused variables, `any`, component
  naming, Vue correctness.
...
- Data fetching in components uses `useFetch` or `useAsyncData`. `$fetch` only inside event handlers, callbacks, and
  server code. Never bare `$fetch` in setup, it double-fetches on hydration.
- Every server route is a `defineEventHandler` with `getValidatedQuery` or `readValidatedBody` and a zod schema. No
  unvalidated input reaches the database.
- Shared request and response types live in `shared/types`, zod schemas in `shared/schemas`, imported by both the route
  and the component. No `any`.
...
- **Card chrome lives outside the flipping element.** `CardFace` rotates one inner div and pins the buttons and the
  rating over it. Putting a control inside a face duplicates it, doubles the tab order, and leaves it spinning mid-flip
  belonging to neither side. The faces carry `overflow-hidden` so the part of speech stripe follows the corner radius
  rather than squaring it off.
- **`v-on="handlers"` runs Vue's `toHandlers()`, which prefixes every key with `on`.** So the object's keys must be bare
  event names (`pointerdown`), not `onPointerdown`. Getting this wrong binds to an event that does not exist and fails
  completely silently: it is what left the stack view with no gestures at all for a whole release, and it is why the
  hand rolled gesture layer is gone.
- **Never put content behind a transition it has to wait for.** `<Transition mode="out-in">` holds the incoming element
  until the outgoing one has finished leaving, and a leave finishes on a frame: in a tab that is not being painted there
  is no frame, so the page keeps showing skeletons after the data has arrived. Animate between peers, and leave loading,
  failure and empty states outside the transition.
- `server/utils/` is auto-imported. Do not write manual imports for it.
- Pages use `definePageMeta` and `useSeoMeta`. No raw `useHead` for standard metadata.
- Composables are named `useX` and live in `app/composables/`. Keep them free of component state assumptions.
- Prefer `useState` for cross-component state. Do not add Pinia without a concrete need.
- Components use `defineProps<{}>()` with types, no runtime prop objects. Multi-word names. Pages are exempt, their
  filenames are routes.
- A failed fetch must render as a failure. Never let an error path fall through to the empty state.

## Styling rules

**Use Nuxt UI as it comes.** The app went through a phase of overriding component slots, adding a 3D press, a raised
panel class and a page gradient, and putting a border on everything. It read as heavy and made problems hard to place,
...
- The page is plain white in light themes and plain black in dark ones. Chenpi is the one exception, because it is a
  reading mode.
...
- Use short hyphens. Never em dashes or en dashes, in any user-facing copy, code comment, commit message, or
  documentation.
...
- `pnpm dev` gets real bindings through Nitro's built-in Cloudflare dev emulation. `nitro-cloudflare-dev` is not needed
  and is not installed. This requires `compatibilityDate` to stay at or after 2025-07-15.
- `wrangler.jsonc` at the root deliberately has no `main` and no `assets`. Nitro fills those in and writes the
  deployable config to `.output/server/wrangler.json`, pointed at by `.wrangler/deploy/config.json`. Declaring them at
  the root gets them ignored with a warning.
- The root `wrangler.jsonc` is still what `wrangler d1` commands and the dev binding proxy read, so bindings belong
  there.
- Local D1 state lives in `.wrangler/state/v3` and is shared between `pnpm dev` and `wrangler d1 execute --local`.
- **Migration SQL is usually hand written here, and that is deliberate.** `drizzle-kit generate` rebuilds a table
  through a `__new_x` copy and a `DROP TABLE`, which silently takes the three `cards_fts` triggers with it and leaves
  the search index mirroring nothing. It also emits `PRAGMA foreign_keys`, which D1 rejects inside a transaction.
  Generate the migration for its snapshot, then read the SQL and replace it if it rebuilds `cards`. Migrations 0002 and
  0006 show the pattern: defer foreign keys, drop and recreate the triggers around the rebuild, and rebuild the FTS
  index afterwards.
- Drizzle prompts interactively when one table is created and another dropped in the same diff, which fails in a non-TTY
  shell. Split the change into two `generate` runs.

## Repo hygiene

- The README is the portfolio piece. Most people will spend ninety seconds on it and never open a source file. It needs
  screenshots, a short recording of a card flipping with the stroke animation, a live demo link, and an architecture
