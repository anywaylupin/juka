...
export default defineEventHandler(async (event): Promise<CardListResponse> => {
  ...
  if (query.groupId !== undefined) {
    conditions.push(
      inArray(
        cards.id,
        db.select({ id: cardGroups.cardId }).from(cardGroups).where(eq(cardGroups.groupId, query.groupId))
      )
    );
  }
  ...
  if (match) {
    conditions.push(sql`${cards.id} in (select rowid from cards_fts where cards_fts match ${match})`);
  }
  ...
  const byCard = await groupsForCards(
    db,
    page.map((row) => row.id)
  );
