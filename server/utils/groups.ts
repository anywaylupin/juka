...
export function toGroup(row: { id: number, name: string, colour: string, orderIndex: number }, count?: number): GroupRecord {
  return {
    id: row.id,
    name: row.name,
    colour: row.colour,
 ...
 */
export async function ownedGroupIds(
  db: JukaDatabase,
  userId: number,
  wanted: number[]
): Promise<number[]> {
...
export async function groupsForCards(
  db: JukaDatabase,
  cardIds: number[]
