 ...
 *
 * Each theme contributes one accent color, pinned to the 500 stop exactly as given in shared/constants/themes.ts.
 ...
 * Now a theme re-points the primary ramp and nothing else: Nuxt UI derives every surface from its own defaults, the page is white or black, and there is one thing per theme that can be wrong instead of fifteen.
 *
...
/**
 * Walks chroma down until the color fits in sRGB, so a vivid hue at an extreme lightness de-saturates instead of clipping to a flat wrong color.
 */
...
// How far each stop sits between the base color and the near-white or near-black end of its ramp.
const LIGHTER = [
...
async function main() {
  ...
  lines.push(
    ...
    ' *',
    ' * One accent ramp per theme, built around the 500 stop in OKLab, plus a single',
    " * shared warm neutral. Surfaces, borders and text come from Nuxt UI's own",
    ' * defaults, so a theme changes the accent and the page color and nothing else.',
    ' */',
    '',
    '@theme static {'
  );
  ...
  for (const theme of THEMES) {
    lines.push(
      ...
      scaleBlock(`--color-${theme.name}`, buildScale(theme.primary)),
      ''
    );
  }
  lines.push('  /* Shared neutral. Warm, never stock grey. */', scaleBlock('--color-taupe', NEUTRAL), '}', '');

  /*
   * Nuxt UI reads --ui-color-primary-* and --ui-color-neutral-*, so switching a theme is a matter of re-pointing the accent.
   * These rules are un-layered on purpose: Nuxt UI ships its defaults inside @layer theme, and an un-layered rule outranks any layered one without needing a specificity trick.
   *
  ...
  for (const theme of THEMES) {
    ...
    lines.push(`/* ${theme.label}: ${theme.description} */`, `${selector} {`);
