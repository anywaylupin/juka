...
async function runWrangler(args: string[]): Promise<Outcome> {
  try {
    const { stdout } = await exec(
      'pnpm',
      ['exec', 'wrangler', 'd1', 'execute', DATABASE, TARGET, '--json', ...args],
...
const values = SAMPLE
  .map(row => `(${row.id}, '${row.hanzi}', '${row.pinyinPlain}', '${row.translation}')`)
  .join(', ')

// Per-character fallback: characters separated by spaces so unicode61 can tokenize them.
const charValues = SAMPLE
  .map(row => `(${row.id}, '${[...row.hanzi].join(' ')}', '${row.pinyinPlain}')`)
  .join(', ')
...
async function scalePhase(): Promise<void> {
  ...
  await timeQuery('per character MATCH 2 char phrase', `select rowid from spike_scale_ch where spike_scale_ch match '"时 间"'`)
  await timeQuery('per character MATCH 3 char phrase', `select rowid from spike_scale_ch where spike_scale_ch match '"图 书 馆"'`)
