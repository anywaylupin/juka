...
const busiestDay = computed(() =>
  Math.max(1, ...calendar.value.flat().map(day => day.count))
)
      ...
      <div class="flex flex-wrap gap-x-4 gap-y-1">
        ...
        >
          <span
            ...
            :style="{ backgroundColor: band.colour, opacity: band.opacity }"
          />
          <span class="text-muted">{{ band.label }}</span>
          <span class="tabular-nums font-medium text-highlighted">{{ band.count }}</span>
        </div>
      </div>
    </section>
        ...
        <div class="space-y-2">
          ...
          >
            <div class="flex items-baseline justify-between gap-2 text-xs">
              ...
              <span class="shrink-0 tabular-nums text-dimmed">{{ row.count }} · {{ percent(row.count) }}%</span>
            </div>
